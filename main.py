...
Pipeline:
  ...
  ✅ Resume system
  ✅ Smart caching
  ✅ WhisperX يحلل الصوت النهائي (الممزوج)
"""
...
def _should_publish_fb(args: argparse.Namespace, content_mode: str) -> bool:
...
def _run_ffmpeg(args: list[str], timeout: int = FFMPEG_TIMEOUT) -> tuple[bool, str]:
    try:
...
def _is_cache_stale(cached: dict, content: str) -> bool:
    ...
    if not cached_tagged:
        log.info(f"  🔄 Cache stale: no tagged sentences (content has {tags_in_content} tags)")
        return True
    if len(cached_tagged) < tags_in_content * 0.5:
        log.info(f"  🔄 Cache stale: {len(cached_tagged)} cached vs {tags_in_content} tags")
        return True
    return False
...
def _inject_tags_into_aligned(aligned: list[dict], tagged: list[dict]) -> list[dict]:
    ...
    for i, seg in enumerate(result):
        log.info(f"     [{i + 1}] [{seg.get('tag', 'information')}] {seg.get('start', 0):.2f}s → {seg.get('end', 0):.2f}s")
    return result
...
def _estimate_sentence_durations(sentences: list[str], total_duration: float) -> list[float]:
    if not sentences:
        return []
    if total_duration <= 0:
    ...
    raw_durations = [max(0.8, total_duration * c / total_words) for c in word_counts]
...
def _build_clip_plan(script_data: dict, ai_data: dict, aligned: list[dict], total_dur: float, content_mode: str = "short") -> tuple[list[list[str]], list[float]]:
    ...
    if aligned and len(aligned) >= len(sentences):
        ...
        for i in range(len(sentences)):
            ...
            next_start = float(aligned[i + 1].get("start", cur_start)) if i < len(sentences) - 1 else total_dur
            dur = max(0.8, round(next_start - cur_start, 3))
            row = _normalize_keywords_row(visual_keywords[i] if i < len(visual_keywords) else [], i)
...
def produce_full_audio(
    script_data: dict, output_base: str, content_mode: str = "short",
    aligned: Optional[list[dict]] = None, music_volume: float = 0.12,
    sfx_type: str = "swoosh",
    ...
    try:
        final_audio = mix_voice_music_sfx(
            voice_path=fallback_voice, content_type=CONTENT_TYPE,
            output_path=mixed_out, clip_durations=clip_dur_list,
            sfx_type=sfx_type, music_volume=music_volume,
...
def run_whisperx(
    ...
    if script_sentences:
        word_timestamps = [w for seg in transcript["aligned"] for w in seg.get("words", [])]
        total_script_words = sum(len(s.split()) for s in script_sentences)
        if word_timestamps and len(word_timestamps) >= 5 and total_script_words > 0:
            diff_ratio = abs(len(word_timestamps) - total_script_words) / total_script_words
            if diff_ratio <= 0.30:
                try:
                    _, rebuilt = build_word_timeline(script_sentences, word_timestamps, transcript["total_duration"])
                ...
                log.warning(f"  ⚠️  Remap skipped — words: {len(word_timestamps)} vs script: {total_script_words}")
...
def _build_manifest(script_data: dict, audio_path: Path, video_paths: list,
    real_dur: float, clip_durations: list[float], aligned: list,
    content_mode: str, mode: str, has_hook: bool = False) -> dict:
...
def _run_remotion_render(manifest_path: Path, output_path: Path) -> None:
    if not RENDER_SCRIPT.exists():
        raise FileNotFoundError(f"render.mjs not found: {RENDER_SCRIPT}")
    try:
        result = subprocess.run(
            ["node", str(RENDER_SCRIPT.resolve()), str(manifest_path), str(output_path)],
...
def produce_bg_video(video_paths: list, audio_path: Path, real_dur: float,
    out_base: str, script_data: dict, has_hook: bool,
    clip_durations: list[float], content_mode: str = "short") -> Path:
    bg_mode = "bg_only" if content_mode == "short" else "long_bg_only"
    suffix = f"_{content_mode}"
    manifest = _build_manifest(script_data, audio_path, video_paths, real_dur,
    ...
    log.info(f"  ✅ BG video [{content_mode.upper()}]: {mb:.1f} MB")
    return output_path
...
def render_words_overlay(bg_video: Path, audio_path: Path, aligned: list,
    sentences: list, script_data: dict, out_base: str,
    content_mode: str = "short") -> Path:
    audio_dur = get_audio_duration(str(audio_path))
    words_mode = "words_only" if content_mode == "short" else "long_words_only"
    suffix = f"_{content_mode}"
    manifest = _build_manifest(
        {**script_data, "sentences": sentences}, audio_path, [bg_video],
        audio_dur, [audio_dur], aligned, content_mode, words_mode,
    ...
    _run_remotion_render(manifest_path, output_path)
    mb = _file_size_mb(output_path)
    log.info(f"  🎉 Final [{content_mode.upper()}]: {output_path.name} ({mb:.1f} MB)")
    return output_path
...
def produce_fb_vertical_version(script_data: dict, audio_path: Path,
    aligned: list, video_paths: list, clip_durations: list[float],
    out_base: str) -> Optional[Path]:
    try:
        ...
        fb_video_paths = fetch_videos_for_script(
            keywords_per_sentence=fb_keywords, clip_durations=clip_durations,
            output_dir=vid_dir_fb, aligned=aligned, content_mode="short")
        bg_fb = produce_bg_video(fb_video_paths, audio_path, real_dur,
            f"{out_base}_fb", script_data, False, clip_durations, "short")
        fb_final = render_words_overlay(bg_fb, audio_path, aligned,
            script_data["sentences"], script_data, f"{out_base}_fb", "short")
...
def get_or_create_ai_data(record: dict, lang: str, tagged: list[dict],
    content_mode: str = "short", force_ai: bool = False, content: str = "") -> dict:
    ...
    if not force_ai and has_ai_cache(cache_key):
        ...
        if cached and cached.get("hook_keyword"):
            if content and _is_cache_stale(cached, content):
                ...
                log.info(f"\n  ♻️  Using cached AI for #{video_number} [{content_mode.upper()}]")
                return cached
    ...
    enricher_record = {"number": video_number, "title": title, "content": content_to_use}
    enriched = enrich_record(record=enricher_record, lang=lang, tagged=tagged, verbose=True)
...
def _build_script_data(record: dict, lang: str, ai_data: dict,
    tagged: list[dict], content_mode: str = "short") -> Optional[dict]:
    if not tagged:
        return None
    ...
    if isinstance(power_words, dict):
        power_words = power_words.get(lang) or power_words.get("ar") or power_words.get("en") or []
    emotion = ai_data.get("analysis", {}).get("primary_emotion", "")
    bg_style = {"fear": "cinematic", "sadness": "cinematic", "awe": "blur"}.get(emotion, "video")
    return {
        "title": record["title"], "display_title": display_title,
        ...
        "full_script": full_script, "sentences": sentences_clean,
        "tagged_sentences": tagged,
        "estimated_seconds": _estimate_duration(full_script, content_mode),
        "word_count": len(full_script.split()), "lang": lang,
        "content_mode": content_mode, "content_type": CONTENT_TYPE,
...
def _do_publish(video_path: str, record: dict, ai_data: dict, lang: str,
    video_number: str, content_mode: str, should_publish_fb: bool,
    should_publish_yt: bool, fb_video_path: str = "", yt_video_path: str = "") -> None:
    ...
    if should_publish_fb:
        if is_published_facebook(video_number, lang, content_mode):
            ...
            try:
                publish_to_facebook(video_path=fb_path, record=record, lang=lang,
                    as_reel=(content_mode == "short"), ai_caption=street_description or title,
    ...
    if should_publish_yt:
        if is_published_youtube(video_number, lang, content_mode):
            ...
            try:
                publish_to_youtube(video_path=yt_path, record=record, lang=lang,
                    street_description=street_description, content_mode=content_mode)
...
def process_video(record: dict, args: argparse.Namespace, out_dir: str,
    should_publish_fb: bool, should_publish_yt: bool,
    content_mode: str = "short") -> dict:
    ...
    if not tagged:
        ...
        return result
    log.info(f"  ✅ Parsed: {len(tagged)} sentences | tags: {', '.join(s.get('raw_tag', '?') for s in tagged)}")
    ...
    save_script_meta(video_number=num, title=title, lang=lang,
        sentences=len(tagged), words=script_data["word_count"], content_mode=content_mode)
    ...
    if args.no_video:
        ...
        try:
            produce_full_audio(script_data, out_base, content_mode)
        except Exception as e:
    ...
    try:
        ...
        audio_path, clean_voice_path, real_dur = produce_full_audio(
            script_data=script_data, output_base=out_base,
            content_mode=content_mode, aligned=None, music_volume=0.12)

        # STEP B: WhisperX
        # ✅ نحلل الصوت النهائي (الممزوج) لضمان تطابق timestamps مع الفيديو
        log.info(f"\n  {'─' * 55}")
        log.info(f"  ✅ STEP B: WhisperX [{mode_label}]")

        whisperx_input = clean_voice_path
        if str(audio_path) != str(clean_voice_path) and audio_path.exists():
            whisperx_input = audio_path
            log.info(f"  📎 Using mixed audio for alignment ({get_audio_duration(str(audio_path)):.1f}s)")

        aligned, whisper_sentences = run_whisperx(
            clean_voice_path=whisperx_input, out_base=out_base,
        ...
        log.info(f"\n  {'─' * 55}")
        log.info(f"  ✅ STEP C: Clip plan + videos [{mode_label}]")
        ...
        log.info(f"\n  {'─' * 55}")
        log.info(f"  ✅ STEP D: Background video [{mode_label}]")
        bg_video = produce_bg_video(video_paths=video_paths, audio_path=audio_path,
        ...
        log.info(f"\n  {'─' * 55}")
        log.info(f"  ✅ STEP E: Words overlay [{mode_label}]")
        final_video = render_words_overlay(bg_video=bg_video, audio_path=audio_path,
        ...
        if content_mode == "long" and should_publish_fb:
            ...
            fb_vertical = produce_fb_vertical_version(
                script_data=script_data, audio_path=audio_path, aligned=aligned,
                video_paths=video_paths, clip_durations=clip_durations, out_base=out_base)
        ...
        mark_render_done(num, lang, str(final_video), real_dur, content_mode,
            fb_path=fb_path, yt_path=yt_path)
        ...
        log.info(f"\n  ✅ Video #{num} ({lang.upper()}) [{mode_label}] → {final_video.name} ({mb:.1f} MB)")
...
def _generate_thumbnails(valid: list[dict], video_results: dict[str, dict],
    args: argparse.Namespace, content_mode: str) -> None:
    thumbnail_queue: list[tuple[str, str]] = []
    for record in valid:
        num = record["number"]
        out_base = str(Path(args.output_dir).resolve() / f"video_{num}_{args.lang}_{content_mode}")
        ...
        try:
            ...
            generate_thumbnail_html(title=record["title"], lang=args.lang, output_path=html_path,
                keyword=hook_keyword, video_paths=video_paths, content_mode=content_mode)
...
def _try_publish_existing(record: dict, args: argparse.Namespace, content_mode: str,
    will_publish_fb: bool, will_publish_yt: bool) -> None:
    num = record["number"]
    lang = args.lang
    suffix = f"_{content_mode}"
    out_base = str(Path(args.output_dir).resolve() / f"video_{num}_{lang}_{content_mode}")
    yt_path = f"{out_base}{suffix}_final.mp4"
    if is_fully_published(num, lang, content_mode):
        log.info(f"  ⏭️  #{num} [{content_mode.upper()}] already published on all platforms")
        return
    ...
    if not Path(fb_path).exists():
        fb_path = yt_path
    _do_publish(video_path=yt_path, record=record, ai_data=ai_data, lang=lang,
        video_number=str(num), content_mode=content_mode,
        should_publish_fb=will_publish_fb and not fb_done,
        should_publish_yt=will_publish_yt and not yt_done,
        fb_video_path=fb_path, yt_video_path=yt_path)


def _handle_management_commands(args: argparse.Namespace) -> bool:
...
def main() -> None:
    ...
    for i, record in enumerate(valid, 1):
        ...
        if not args.force and is_render_done(record["number"], lang, content_mode):
            _try_publish_existing(record=record, args=args, content_mode=content_mode,
                will_publish_fb=will_publish_fb, will_publish_yt=will_publish_yt)
            continue
        try:
            result = process_video(record=record, args=args, out_dir=args.output_dir,
                should_publish_fb=will_publish_fb, should_publish_yt=will_publish_yt,
                content_mode=content_mode)
