...
def _gemini(prompt: str, temperature: float = 0.25, max_tokens: int = 3000) -> str:
    client   = _get_client()
...
- Every sentence is translated"""
    ...
    raw = _gemini(prompt, temperature=0.25, max_tokens=3000)
    ...
    if trans_count < n_sentences:
        print(f"  ⚠️  Translation: {trans_count}/{n_sentences} sentences — retrying...")
...
{{
  ...
  "full_script": "<complete script in {lang_name}>"
    ...
    raw = _gemini(prompt, temperature=0.2, max_tokens=2000)
    raw = re.sub(r"^```(?:json)?\s*", "", raw)
    raw = re.sub(r"\s*```$",          "", raw)
    ...
    print(f"  ✅ Retry: {len(all_sentences)} sentences total")
    return new_data
