...
// ✨ حجم كبير جداً — تملأ الشاشة
// ✨ ألوان ديناميكية حسب المشاعر
// ✨ Power Words بلون وحجم مختلف
...
const EMOTION_COLORS = {
  curiosity: { word: "#FFD700", glow: "rgba(255,215,0,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FF1744", label: "Golden" },
  fear:      { word: "#FF4444", glow: "rgba(255,68,68,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFD700", label: "Fear"   },
  hope:      { word: "#00E676", glow: "rgba(0,230,118,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFFFFF", label: "Hope"   },
  joy:       { word: "#FF9100", glow: "rgba(255,145,0,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFFFFF", label: "Joy"    },
  awe:       { word: "#AA00FF", glow: "rgba(170,0,255,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFD700", label: "Awe"    },
  surprise:  { word: "#00B0FF", glow: "rgba(0,176,255,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFD700", label: "Surprise" },
  desire:    { word: "#FF1744", glow: "rgba(255,23,68,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFD700", label: "Desire" },
  anger:     { word: "#FF1744", glow: "rgba(255,23,68,0.6)",   shadow: "rgba(0,0,0,0.9)", power: "#FFD700", label: "Anger"  },
  sadness:   { word: "#82B1FF", glow: "rgba(130,177,255,0.6)", shadow: "rgba(0,0,0,0.9)", power: "#FFFFFF", label: "Sadness" },
  default:   { word: "#FFFFFF", glow: "rgba(255,255,255,0.5)", shadow: "rgba(0,0,0,0.9)", power: "#FF1744", label: "Default" },
};
...
function buildFrameStateMap() {
  // استخراج كل الكلمات مع أوقاتها من aligned
  const allWords = [];
  ...
  if (allWords.length > 0) {
    console.log(`   First: [${allWords[0].start.toFixed(2)}s] "${allWords[0].word}"`);
    console.log(`   Last:  [${allWords[allWords.length-1].start.toFixed(2)}s] "${allWords[allWords.length-1].word}"`);
  }

  // بناء frame map: لكل frame → أي كلمة تُعرض
  ...
  for (let f = 0; f < totalFrames; f++) {
    ...
    // إذا لم نجد → ابحث عن آخر كلمة انتهت
    if (!currentWord) {
      for (let i = allWords.length - 1; i >= 0; i--) {
        if (t >= allWords[i].end) {
          // بين كلمتين → أبقِ الأخيرة لفترة قصيرة
    ...
    if (currentWord) {
      // حساب progress داخل الكلمة (0→1)
      const wordDur  = Math.max(currentWord.end - currentWord.start, 0.05);
      const progress = Math.min((t - currentWord.start) / wordDur, 1.0);
...
function buildHTML(opts) {
  ...
  // ✨ Scale animation: الكلمة تكبر عند الظهور ثم تستقر
  // progress: 0→1 خلال مدة الكلمة
  const scaleIn   = progress < 0.15
    ? 0.7 + (progress / 0.15) * 0.3   // 0.7→1.0 في أول 15%
    : 1.0;
  ...
  // ✨ تأثير glow أقوى للـ power words
  const glowSize  = isPower ? "80px" : "40px";
  const glowSize2 = isPower ? "120px" : "60px";

  // ✨ خلفية خلف الكلمة (pill shape) — للـ power words فقط
  const pillBg = isPower
    ? `background:rgba(255,0,0,0.85);padding:20px 50px;border-radius:9999px;`
  ...
  const titleFontSize = titleAr ? 38 : 34;
  ...
  const hookText    = custom_hook && custom_hook.trim().length > 0
    ? custom_hook.trim()
...
<head>
  ...
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    /* العنوان — صغير وأنيق فوق */
    .title-container {
      position:absolute;
      top:${isHookFrame ? "280px" : "340px"};
      left:50%;
    ...
    .title-box {
      ...
      padding:12px 30px;
      border-radius:9999px;
      box-shadow:0 0 30px rgba(255,0,0,0.5),0 6px 20px rgba(0,0,0,0.4);
    }
    .title-text {
    ...
    /* ✨ الكلمة الواحدة — في المنتصف بالضبط */
    .word-container {
      position:absolute;
      left:50%; top:55%;
    ...
    .word-pill {
      display:inline-block;
      ${pillBg}
      box-shadow:${isPower ? "0 0 60px rgba(255,0,0,0.7),0 12px 35px rgba(0,0,0,0.6)" : "none"};
    }
    .word-text {
      font-family:${font};
      ...
      text-shadow:
        ...
        0 4px 20px ${shadowClr},
        0 0 5px ${shadowClr};
      letter-spacing:${ar ? "0px" : "2px"};
    }
  </style>
</head>
<body>
...
async function renderAllPNGs(page, frameStateMap) {
  ...
  for (let f = 0; f < frameStateMap.length; f++) {
    ...
    if (!state) {
      ...
      if (!uniqueStates.has(key))
        uniqueStates.set(key, { word: null, isHookFrame, globalFrame: f });
      continue;
    }

    // Scale stage: 3 مراحل (0-15%, 15-100%, 100%)
    const scaleStage = state.progress < 0.15 ? Math.floor(state.progress * 20) : "done";
    ...
    if (!uniqueStates.has(key))
      uniqueStates.set(key, { ...state, isHookFrame, globalFrame: f });
  }
  ...
  for (const [initWord, initLang] of [["مرحبا", "ar"], ["Hello", "en"]]) {
    const html = buildHTML({
      word: initWord, progress: 1.0, isPower: false,
      isHookFrame: false, globalFrame: TITLE_SLIDE_FRAMES,
    });
  ...
  for (const [key, state] of uniqueStates) {
    ...
    if (rendered % 40 === 0 || rendered === uniqueStates.size)
...
function buildFrameDir(clipFrameMap, pngCache, idx, clipStartFrame) {
  ...
  for (let f = 0; f < clipFrameMap.length; f++) {
    ...
    const slideKey    = globalFrame < TITLE_SLIDE_FRAMES ? `sl${globalFrame}` : "sld";
...
function processBackground(videoPath, duration, outPath, idx, isHook = false) {
  ...
  if (isHook) {
    ...
    zoomFilter =
      `scale=w='trunc((iw*1.3)/2)*2':h='trunc((ih*1.3)/2)*2',` +
      `zoompan=z='max(zoom-0.0008,1.0)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':` +
      `d=${Math.ceil(duration * FPS)}:s=${WIDTH}x${HEIGHT}:fps=${FPS}`;
  } else if (motionType === 2) {
    zoomFilter =
      `scale=w='trunc((iw*1.2)/2)*2':h='trunc((ih*1.2)/2)*2',` +
      `zoompan=z='1.1':x='if(gte(x,iw/10),x-0.5,iw/10)':y='ih/2-(ih/zoom/2)':` +
      `d=${Math.ceil(duration * FPS)}:s=${WIDTH}x${HEIGHT}:fps=${FPS}`;
  } else {
    zoomFilter =
      `scale=w='trunc((iw*1.2)/2)*2':h='trunc((ih*1.2)/2)*2',` +
      `zoompan=z='1.1':x='if(lte(x,iw-iw/10),x+0.5,iw-iw/10)':y='ih/2-(ih/zoom/2)':` +
      `d=${Math.ceil(duration * FPS)}:s=${WIDTH}x${HEIGHT}:fps=${FPS}`;
  }
  ...
  let r = spawnSync("ffmpeg", [
    ...
    "-vf", `${zoomFilter},${cinematic},${fade}`,
    "-r", String(FPS), "-c:v", "libx264", "-preset", "fast",
    "-crf", isHook ? "17" : "19", "-pix_fmt", "yuv420p", "-an", outPath,
  ], { stdio: ["ignore", "pipe", "pipe"] });
  ...
  if (r.status !== 0) {
    ...
    r = spawnSync("ffmpeg", [
      ...
      "-r", String(FPS), "-c:v", "libx264", "-preset", "fast", "-crf", "21",
    ...
    if (r.status !== 0) {
      spawnSync("ffmpeg", [
        ...
        "-r", String(FPS), "-c:v", "libx264", "-preset", "fast", "-crf", "23",
        "-pix_fmt", "yuv420p", "-an", outPath,
      ], { stdio: ["ignore", "pipe", "pipe"] });
    }
  }
...
function xfadeConcat(clipPaths, clipDurations) {
  ...
  const TRANSITIONS = ["fade","fadeblack","fadegrays","smoothleft","smoothright","circlecrop"];
  const XFADE = 0.5;
  const filters = [];
  ...
  const r = spawnSync("ffmpeg", [
    ...
    "-map", "[vout]", "-c:v", "libx264", "-preset", "fast", "-crf", "19",
    "-pix_fmt", "yuv420p", "-an", outPath,
  ], { stdio: ["ignore", "pipe", "pipe"] });
