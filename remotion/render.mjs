// remotion/render.mjs
// ═══════════════════════════════════════════════════════════════════════════
// 🎬 Video Renderer — Cinematic Edition v5 (Production Ready)
//
// Fixes from v4:
//   ✅ 1. TMP cleanup شامل — process.on(exit/SIGINT/SIGTERM/uncaught)
//   ✅ 2. concat demuxer — escape single quotes في المسارات
//   ✅ 3. runFFmpeg — slice(-2000) بدل slice(-300)
//   ✅ 4. applyMetadata — fallback لـ libx264/aac عند فشل copy
//   ✅ 5. buildDramaticLightingFilter — curves+eq بدل geq (أسرع)
//   ✅ 6. buildLongBgVideo — normalize للفيديو الأصلي قبل push
//   ✅ 7. mergeAudio — تحقق من وجود audioPath
//   ✅ 8. _durationCache.clear() بعد انتهاء render
// ═══════════════════════════════════════════════════════════════════════════
...
const safeOut = outputPath
  ...
  .replace(/_+/g, "_")
  .slice(-22);
const TMP = join(tmpdir(), `vsg_${safeOut}_${process.pid}`);
mkdirSync(TMP, { recursive: true });

// ✅ تتبع ما إذا كان الـ render اكتمل بنجاح
let _renderCompleted = false;

function _cleanup(exitCode) {
  // ✅ تنظيف TMP دائماً — سواء نجح أو فشل
  try {
    rmSync(TMP, { recursive: true, force: true });
    console.log(`  🗑️  TMP cleaned: ${TMP}`);
  } catch {}

  // ✅ تنظيف الـ duration cache
  _durationCache.clear();
}

// ✅ تسجيل handlers لكل حالات الخروج
process.on("exit",              (code) => _cleanup(code));
process.on("SIGINT",            ()     => { _cleanup(130); process.exit(130); });
process.on("SIGTERM",           ()     => { _cleanup(143); process.exit(143); });
...
function buildiPhoneMetadata() {
  ...
  return [
    ...
    "-metadata", "com.apple.quicktime.model=iPhone 16 Pro Max",
    "-metadata", "com.apple.quicktime.software=iOS 18.3",
    ...
    "-metadata", "lens=Apple iPhone 16 Pro Max back camera 9mm f/2.8",
...
function runFFmpeg(args, label) {
  ...
  if (r.status !== 0) {
    // ✅ slice(-2000) بدل slice(-300)
    const stderr = r.stderr
      ? r.stderr.toString().slice(-2000)
      : "no stderr";
    console.log(
      `  ⚠️ FFmpeg${label ? ` [${label}]` : ""} failed: ` +
      stderr
    );
  }
  return r;
...
// ✅ escape single quotes للـ concat demuxer
function escapePathForConcat(filePath) {
  // في concat demuxer، المسارات بين single quotes
  // نحتاج escape لأي single quote في المسار
...
// ✅ applyMetadata مع fallback
function applyMetadata(inp, out) {
  if (!inp || !existsSync(inp)) {
    console.log(`  ⚠️ applyMetadata: input not found: ${inp}`);
    return;
  }
  ...
  // ✅ محاولة 2: fallback لـ libx264/aac عند فشل copy
  console.log(
    "  ⚠️ Metadata copy failed — retrying with re-encode"
  );
  const r2 = runFFmpeg(
    [
      "-y", "-i", inp,
  ...
  if (r2.status === 0 && existsSync(out)) {
    console.log(
      `  ✅ Metadata (re-encoded): 📱 iPhone | 📍 ${location.city}`
    );
    return;
  }

  // محاولة 3: copy بدون metadata
  console.log(
    "  ⚠️ Metadata re-encode failed — copying as-is"
  );
...
// ✅ scale بسيط — متوافق مع كل إصدارات ffmpeg
...
// ✅ curves+eq بدل geq — أسرع بكثير على CPU
function buildDramaticLightingFilter() {
  return (
    // تعتيم طفيف + تباين + لون دافئ
    `curves=` +
...
// ═══════════════════════════════════════════════════════════════════════════
// ✅ normalizeClip — دالة مشتركة للـ Short و Long
// ═══════════════════════════════════════════════════════════════════════════

function normalizeClip(videoPath, outputPath, durationSec, idx) {
  """
  تحويل أي فيديو إلى: WIDTH×HEIGHT, FPS, yuv420p.
  يضمن أن iw=WIDTH و ih=HEIGHT قبل أي effects.
  """
  const srcDur    = probeDuration(videoPath);
  const loopArgs  = srcDur > 0 && srcDur < durationSec * 1.3
    ? ["-stream_loop", "-1"]
    : [];

  const r = runFFmpeg([
    "-y", ...loopArgs, "-i", videoPath,
    "-t", (durationSec * 1.5).toFixed(3),
    "-vf", [
  ...
  if (r.status !== 0 || !existsSync(outputPath)) {
    // ✅ Fallback: scale بسيط بدون force_original
    runFFmpeg([
...
function processBackground(videoPath, dur, outputFile, idx, isHookClip) {
  ...
  try {
    ...
    // STEP 2: Effects
    const effectsInput = (normalizeOk && existsSync(normOut))
      ? normOut
      : videoPath;

    const vfParts = [];
    // لو الـ input ليس normalized → أضف scale
    if (!normalizeOk) {
      vfParts.push(
        `scale=${WIDTH}:${HEIGHT}:force_original_aspect_ratio=increase`
      );
    ...
    if (r.status !== 0 || !existsSync(stage1)) {
      // Fallback: use normOut or source
      const fallback = existsSync(normOut) ? normOut : effectsInput;
      if (existsSync(fallback)) {
        copyFileSync(fallback, stage1);
        console.log(
          `  ⚠️ Effects fail [${idx}] — using normalized`
        );
      } else {
        // آخر fallback: scale مباشر
        runFFmpeg([
          "-y", "-stream_loop", "-1", "-i", videoPath,
          "-t", d.toFixed(3),
          "-vf", [
            `scale=${WIDTH}:${HEIGHT}`, "setsar=1", `fps=${FPS}`,
          ].join(","),
          "-c:v", "libx264", "-preset", "fast",
          "-crf", "23", "-pix_fmt", "yuv420p", "-an",
          stage1,
        ], `effects-last-fallback[${idx}]`);
      }
    } else {
    ...
    runFFmpeg([
      "-y", "-i", existsSync(stage1) ? stage1 : normOut,
      "-t", d.toFixed(3),
      "-c", "copy",
      trimOut,
    ], `trim[${idx}]`);
    ...
    if (r2.status !== 0 || !existsSync(outputFile)) {
      const fallback = existsSync(srcTrim) ? srcTrim
        : existsSync(normOut) ? normOut
        : null;
    ...
    try { spawnSync("rm", ["-f", trimOut], { stdio: "ignore" }); } catch {}
  }
...
function buildLongBgVideo(clipPlan, audioPath, outputFile) {
  ...
  for (const clip of clipPlan) {
    ...
    process.stdout.write(
      `  [${i + 1}/${clipPlan.length}] ${d.toFixed(1)}s... `
    );

    // ✅ استخدام normalizeClip المشتركة
    const normalizeOk = normalizeClip(v, normOut, srcNeeded, i);

    if (normalizeOk) {
      // تطبيق effects على normalized clip
      ...
      const useFile = (rEffects.status === 0 && existsSync(effectsOut))
        ? effectsOut
        : normOut;
      ...
      if (rTrim.status === 0 && existsSync(trimOut)) {
        ...
        try { spawnSync("rm", ["-f", normOut, effectsOut], { stdio: "ignore" }); } catch {}
      } else {
      ...
      // ✅ Fallback — لكن normalize الفيديو الأصلي أولاً
      console.log(`⚠️ → normalized fallback`);
      const fallOut = join(
        TMP, `lf_${String(i).padStart(3, "0")}.mp4`
      );
      // ✅ دائماً نمرر الفيديو الأصلي عبر normalize
      // لضمان نفس الأبعاد مع بقية المقاطع
      runFFmpeg([
        "-y", "-stream_loop", "-1", "-i", v,
        "-t", d.toFixed(3),
        "-vf", [
      ...
      // ✅ إذا فشل حتى الـ fallback → لا نضيف الفيديو الأصلي
      // بل نُعيد استخدام آخر مقطع ناجح
      if (existsSync(fallOut)) {
        normalizedClips.push(fallOut);
      } else if (normalizedClips.length > 0) {
        console.log(
          `  ⚠️ Complete fail [${i}] — reusing last clip`
        );
  ...
  try {
    ...
    if (rXfade.status === 0 && existsSync(concatXfade)) {
      console.log("  ✅ Cinematic xfade done");
...
function _finalizeLongVideo(videoSource, audioPath, outputFile) {
  ...
  console.log(
    `🎵 Audio:${ad.toFixed(1)}s | Video:${vd.toFixed(1)}s`
  );
  ...
  if (vd < ad - 0.3) {
    ...
    if (rLoop.status === 0 && existsSync(looped)) {
      videoInput = looped;
    }
  }
...
function buildAssFile() {
  ...
  const wordSize  = (content_mode === "long" && platform === "yt")
    ? 70 : 85;
  const titleSize = (content_mode === "long" && platform === "yt")
    ? 36 : 46;

  const wordMarginV  = Math.floor(HEIGHT * 0.40);
  const titleMarginV = (content_mode === "long" && platform === "yt")
    ? 30 : 400;
  ...
  const header = [
    ...
    `-1,0,0,0,100,100,0,0,1,4,2,2,80,80,${wordMarginV},1`,

    `Style: Title,${fontName},${titleSize},` +
    `&H00FFFFFF,&H000000FF,&H00000000,&H80000000,` +
    `-1,0,0,0,100,100,0,0,1,2,1,8,40,40,${titleMarginV},1`,
...
function buildLongWordsOverlay(bgVideoPath, audioPath, outputFile) {
  ...
  if (r.status !== 0) {
    console.log("  ⚠️ ASS overlay failed — using bg as-is");
    if (bgHasAudio) {
  ...
  try {
    spawnSync("rm", ["-f", tmpOut, assFile], { stdio: "ignore" });
  } catch {}
...
function detectVideoSections() {
  if (!aligned || aligned.length === 0) {
    ...
    return Array.from({ length: totalClips }, (_, i) => ({
      ...
      start: 0, end: 0, idx: i,
      tag:   i === 0 ? "intrigue"
           : i === totalClips - 1 ? "confident" : "information",
    }));
  }
...
function buildClipPlan() {
  ...
  const syncedSections = Array.from({ length: count }, (_, i) =>
    sections[i] || {
      ...
      tag:   i === 0 ? "intrigue"
           : i === count - 1 ? "confident" : "information",
      start: 0, end: 0, idx: i,
    }
  );
  ...
  const plan = durations.map((d, i) => {
    ...
    if (nextSec) {
      ...
      trans = {
        ...
        type:     transType,
        duration: isMajor ? XFADE_DUR : XFADE_DUR * 0.56,
      };
    }
...
function mergeAudio(videoPath, audioPath, outputFile) {
  // ✅ تحقق من وجود audioPath
  if (!audioPath || !existsSync(audioPath)) {
    console.log(`  ⚠️ mergeAudio: audio not found: ${audioPath}`);
    console.log("  ⚠️ Copying video without audio");
  ...
  console.log(
    `🎵 Audio:${ad.toFixed(3)}s | Video:${vd.toFixed(3)}s`
  );
...
function concatClipsWithTransitions(processedClips, clipPlan) {
  ...
  for (const group of groups) {
    ...
    const totDur   = clips.reduce((s, c) => s + c.dur, 0) -
                     X * (clips.length - 1);
  ...
  for (let i = 1; i < groupOutputs.length; i++) {
    ...
    const transDur  = (trans && trans.duration) || XFADE_DUR;
...
function overlayOnBg(bgVideo, captionMov, audioPath, outputFile) {
  ...
  if (r.status !== 0) {
    console.log("  ⚠️ Overlay failed — using bg video");
...
function stateKey(state, gf, ts) {
  ...
  if (ts) {
    ...
    const wk = state ? safeKey(state.word, 10) : "empty";
    return `tr_${ts.tag}_${pb}_${wk}_${state && state.isPower ? 1 : 0}`;
  }
...
function buildHTMLShort(params) {
  ...
  const progress        = params.progress !== undefined
    ? params.progress : 0.5;
  ...
  const wts = isPower
    ? (
        `font-family:${font};font-size:${fs}px;font-weight:900;` +
  ...
  return (
    `<!DOCTYPE html><html lang="${la}"><head>` +
    ...
    `rgba(0,0,0,0.90) 0%,rgba(0,0,0,0.55) 50%,transparent 100%);` +
    `pointer-events:none;z-index:1;}` +
    `.ob{position:absolute;bottom:0;left:0;right:0;height:45%;` +
    `background:linear-gradient(to top,` +
    `rgba(0,0,0,0.92) 0%,rgba(0,0,0,0.50) 65%,transparent 100%);` +
    `pointer-events:none;z-index:1;}` +
    `.flash{position:absolute;inset:0;` +
    `background:${tr.flashColor};` +
    `opacity:${tr.flashOpacity.toFixed(4)};` +
    `pointer-events:none;z-index:50;}` +
    `.tc{position:absolute;top:410px;left:50%;width:92%;` +
    `max-width:980px;direction:${td};text-align:center;` +
    `z-index:30;transform:translateX(-50%) ` +
    `translateY(${ta.translateY.toFixed(2)}px);` +
    `opacity:${ta.opacity.toFixed(4)};}` +
    `.tc::after{content:'';display:block;margin:16px auto 0;` +
    `width:120px;height:4px;border-radius:2px;` +
    `background:linear-gradient(90deg,transparent,#FF1744,transparent);` +
    `opacity:${ta.opacity.toFixed(4)};}` +
    `.tt{font-family:${tf};font-size:${tfs}px;font-weight:900;` +
    `color:#FFF;display:inline-flex;align-items:center;` +
    `justify-content:center;gap:14px;line-height:1.3;` +
    `direction:${td};-webkit-text-stroke:2px rgba(0,0,0,0.8);` +
    `paint-order:stroke fill;` +
    `text-shadow:0 0 30px rgba(255,23,68,0.6),` +
    `0 4px 20px rgba(0,0,0,0.9),2px 2px 0 rgba(0,0,0,0.8);}` +
    `.te{font-size:${es}px;-webkit-text-stroke:0;}` +
    `.hb{position:absolute;top:${tia ? "290px" : "270px"};` +
    `left:50%;transform:translateX(-50%);` +
    `background:linear-gradient(135deg,` +
    `rgba(220,0,0,0.95),rgba(160,0,0,0.95));` +
    `color:#fff;font-family:${hf};` +
    `font-size:${ha ? "32px" : "28px"};font-weight:900;` +
    `padding:12px 38px;border-radius:9999px;z-index:25;` +
    `white-space:nowrap;direction:${hd};` +
    `border:2px solid rgba(255,120,120,0.4);` +
    `box-shadow:0 0 50px rgba(220,0,0,0.7),` +
    `0 8px 24px rgba(0,0,0,0.5);}` +
    `.wc{position:absolute;left:50%;top:54%;` +
    `transform:${wt};opacity:${fo.toFixed(4)};` +
...
async function handleBgOnlyMode() {
  ...
  if (isLong) {
    ...
    console.log(
      `\n🎉 BG [${content_mode.toUpperCase()}/${platform.toUpperCase()}]` +
      ` → ${outputPath}\n`
    );
...
async function handleWordsOnlyMode() {
  ...
  try {
    ...
    // ✅ TMP يتم تنظيفه في process.on("exit")
    // لا نحتاج rmSync هنا لأنه سيُنفَّذ تلقائياً
  }
}
...
main().catch(e => {
  ...
  // ✅ _cleanup يُستدعى تلقائياً عبر process.on("exit")
  process.exit(1);
});
