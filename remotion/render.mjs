...
function buildWordList() {
  ...
  // Fallback: توزيع متساوٍ فقط إذا لم يوجد alignment
...
function buildHTML({ word, isPower = false, isHook = false, globalFrame = 0 }) {
  ...
  const titleFnt = getFontFamily(display_title);
  ...
  const hookFnt  = getFontFamily(hookText);
...
.hb{position:absolute;top:155px;left:50%;transform:translateX(-50%);background:rgba(210,0,0,0.93);color:#fff;font-family:${hookFnt};font-size:${hookAr?"34px":"30px"};font-weight:900;padding:13px 38px;border-radius:9999px;z-index:25;white-space:nowrap;direction:${hookDir};box-shadow:0 0 40px rgba(210,0,0,0.85),0 6px 22px rgba(0,0,0,0.55)}
.tc{position:absolute;top:${isHook?"278px":"338px"};left:50%;width:90%;max-width:960px;direction:${titleDir};text-align:center;z-index:20;transform:translateX(calc(-50% + ${slideX.toFixed(2)}px));opacity:${titleOp.toFixed(4)}}
...
function processBackground(videoPath, duration, outPath, idx, isHook = false) {
  ...
  const hookZoom = `scale=w='trunc((iw*1.5)/2)*2':h='trunc((ih*1.5)/2)*2',zoompan=z='min(zoom+0.0015,1.5)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${frames}:s=${WIDTH}x${HEIGHT}:fps=${FPS}`;
