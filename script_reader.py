...
Supports extended columns: hooks, funnel stages (TOFU/MOFU/BOFU).
"""
import csv
import re
...
def _detect_columns(headers: list[str]) -> dict[str, int]:
    ...
    defaults = {"number": 0, "title": 1, "ar_content": 2, "en_content": 3}
...
def _read_excel(path: Path) -> list[dict]:
    ...
    print(f"  📊 Detected: {[f for f in col_map]}")
...
def split_into_sentences(text: str, lang: str = "en") -> list[str]:
    if not text or not text.strip():
        return []
    # Split on sentence-ending punctuation (Arabic + Latin) and newlines
    if lang == "ar":
        parts = re.split(r"(?<=[.!?؟\u06D4])\s+|\n+", text.strip())
    else:
        parts = re.split(r"(?<=[.!?])\s+|\n+", text.strip())
    ...
    for p in parts:
        ...
        if len(p) < 4:
            continue
        # Merge ultra-short sentences into next
